...
2. 协议初筛：只留 `vless` / `trojan` / `vmess` / `hysteria2`，其余（`http`、`socks5`、`ss` 等）直接丢弃，不进测速
3. 按 `exclusion` / `inclusion` 关键词过滤节点名称和地址
4. 把所有源的节点汇总成一批，一次性并发测延迟
5. 只保留延迟低于 **500ms** 的节点，按延迟升序排列
6. 节点名加上订阅源前缀（`FREESUB-香港01`），追加进模板并写出配置

协议白名单是 `helper.py` 里的 `ALLOWED_TYPES` 常量，匹配时把 `type` 转小写后精确比对。改成空集合 `frozenset()` 即关闭初筛。
